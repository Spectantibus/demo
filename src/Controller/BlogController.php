...
class BlogController extends Controller
{
    ...
    public function postShowAction(Post $post): Response
    {
        ...
        $note = $this->getDoctrine()->getManager()->getRepository(Note::class)->findOneBy([
            "user" => $this->getUser(),
            "post" => $post,
        ]);
...
/**
 * @Route("/note/{postSlug}", name="note_new")
 * @Method("POST")
 * @Security("is_granted('IS_AUTHENTICATED_FULLY')")
 * @ParamConverter("post", options={"mapping": {"postSlug": "slug"}})
 */
    public function noteNewAction(Request $request, Post $post, EventDispatcherInterface $eventDispatcher): Response
    {

        $user = $this->getUser();

        $noteRep = $this->getDoctrine()->getManager()->getRepository(Note::class);
        $note = $noteRep->findOneBy([
            "user" => $user,
            "post" => $post,
        ]);
        if(!isset($note))
        ...
        return $this->render('blog/note_form_error.html.twig', [
            'post' => $post,
            'form' => $form->createView(),
        ]);
    ...
    public function noteFormAction(Post $post): Response
    {
        $note = $this->getDoctrine()->getManager()->getRepository(Note::class)->findOneBy([
            "user" => $this->getUser(),
            "post" => $post,
        ...
        return $this->render('blog/_note_form.html.twig', [
            'post' => $post,
            'form' => $form->createView(),
        ]);
    }
